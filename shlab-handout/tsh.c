...
{   
    ...
    int is_bulitin = builtin_cmd(argv);
    if (!is_bulitin) {
        // Is not builtin_cmd
        if ((pid = fork()) == 0) {
            // Child process
            ...
            if (!bg) {
                ...
                //fflush(stdout); // flush stdout before print
...
{   
    ...
    if (strcmp(argv[0], "quit") == 0) clean_exit();
...
{   
    ...
    while (pid == fgpid(jobs) && getjobpid(jobs, pid)->state != ST) {
